...
public class AdminDashboardPanel extends JPanel implements GlobalSearchable {
    ...
    public static boolean validateDateFormat(String date) {
        ...
        if (!isNumeric(yearStr) || !isNumeric(monthStr) || !isNumeric(dayStr)) return false;
 ...
 // HOSPITAL SERVICE PANEL -------------------------------------------
    ...
    private void openAddUserDialog() {
        ...
        JTextField passwordField = new JPasswordField(); passwordField.setEditable(false); passwordField.setToolTipText("Password will be auto-generated on Save (e.g. PW000001)");
        ...
        addPersonal.accept("Surname:", surnameField); addPersonal.accept("Given Name:", givenNameField); addPersonal.accept("Middle Name:", middleField); addPersonal.accept("Date Of Birth (YYYY-MM-DD):", dobField);
        ...
        addProf.accept("License / PRC ID:", licenseField); addProf.accept("PRC Expiry Date(YYYY-MM-DD):", prcExpiryField);
        left.add(prof);

        // Identification block
        JPanel idBlock = new JPanel(new GridBagLayout()); idBlock.setBackground(Color.WHITE);
        idBlock.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(COLOR_BORDER), "Identification"));
        ...
        acgbc.gridy++; acgbc.gridx=0; acgbc.weightx=0; accountPanel.add(new JLabel("Password: (Auto-Generated Password)"), acgbc); acgbc.gridx=1; acgbc.weightx=1; passwordField.setPreferredSize(new Dimension(320,28)); accountPanel.add(passwordField, acgbc);
        ...
        okB.addActionListener(e -> {
        	...
        	if (!requireValue(dlg, surnameField, "Surname")) return;
        	if (!requireValue(dlg, givenNameField, "Given Name")) return;
        	if (!requireValue(dlg, dobField, "Date of Birth")) return;
        	...
        	if (genderBox.getSelectedItem() == null) {
        	    JOptionPane.showMessageDialog(dlg, "Please select a Gender.", "Missing Required Field", JOptionPane.WARNING_MESSAGE);
        	    return;
        	}
        	if (!requireValue(dlg, nationalityField, "Nationality")) return;

        	// === CONTACT REQUIRED FIELDS ===
        	if (!requireValue(dlg, emailField, "Email Address")) return;
        	if (!requireValue(dlg, contactNumField, "Contact Number")) return;
        	if (!requireValue(dlg, addressField, "Address")) return;
        	if (!requireValue(dlg, emergencyNameField, "Emergency Contact Name")) return;
        	if (!requireValue(dlg, emergencyContactField, "Emergency Contact Number")) return;
        	...
        	if (chosenRole == Role.DOCTOR) {
        	    if (!requireValue(dlg, specialityField, "Speciality / Department")) return;
        	    if (!requireValue(dlg, licenseField, "License Number")) return;
        	    if (!requireValue(dlg, prcExpiryField, "PRC Expiry Date")) return;
        	...
        	if (chosenRole == Role.STAFF) {
        	    if (!requireValue(dlg, titleField, "Staff Title / Position")) return;
        	}
        	...
        	if (chosenRole == Role.PATIENT) {
        	    if (!minorCheck.isSelected()) {
        	        ...
        	        if (!requireValue(dlg, idNumberField, "ID Number")) return;
        	    } else {
        	        // If minor, require student ID
        	        if (!requireValue(dlg, studentIdField, "Student ID")) return;
        	    }
        	}
