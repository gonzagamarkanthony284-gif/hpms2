...
public class DatabaseInitializer {
    ...
    private static boolean testUserCRUD() {
        try {
            ...
            // CREATE
            Model.User user = new Model.User("testuser", "testuser@example.com", "Test User",
            ...
            if (updated.isEmpty() || !updated.get().getEmail().equals("updated@example.com")) return false;
            
            // DELETE
            if (!repo.delete(saved.getId())) return false;
    ...
    private static boolean testPatientCRUD() {
        try {
            ...
            Model.User user = new Model.User("testpatient", "testpatient@example.com", "Test Patient",
                                             Util.PasswordHasher.hash("TestPassword123".toCharArray()), Model.Role.PATIENT);
            Model.User savedUser = userRepo.save(user);
            
            // CREATE
            java.time.LocalDate dob = java.time.LocalDate.of(1990, 1, 1);
            Model.Patient patient = new Model.Patient(savedUser, "PT-TEST-001", dob, "M",
            ...
            // DELETE
            if (!patientRepo.delete(saved.getId())) return false;
    ...
    private static boolean testDoctorCRUD() {
        try {
            ...
            Model.User user = new Model.User("testdoctor", "testdoctor@example.com", "Test Doctor",
            ...
            if (saved == null || saved.getDoctorId() == null) return false;
            ...
            // DELETE
            if (!doctorRepo.delete(saved.getDoctorId())) return false;
    ...
    private static boolean testStaffCRUD() {
        try {
            ...
            // Create associated user
            Model.User user = new Model.User("teststaff", "teststaff@example.com", "Test Staff",
            ...
            if (saved == null || saved.getStaffId() == null) return false;
            ...
            // DELETE
            if (!staffRepo.delete(saved.getStaffId())) return false;
    ...
    private static boolean testAppointmentCRUD() {
        try {
            ...
            // Create test patient
            Model.User patientUser = new Model.User("appointpatient", "appointpatient@example.com",
            ...
            java.time.LocalDate dob = java.time.LocalDate.of(1990, 1, 1);
            Model.Patient patient = new Model.Patient(savedPatientUser, "PT-APP-001", dob, "M",
                                                      "O+", "Single", "123 Test St", "555-1234",
                                                      "John Doe", "555-0000");
            Model.Patient savedPatient = patientRepo.save(patient);
            
            // Create test doctor
            Model.User doctorUser = new Model.User("appointdoctor", "appointdoctor@example.com",
            ...
            // DELETE
            if (!appointmentRepo.delete(saved.getId())) return false;
    ...
    private static boolean testBillingCRUD() {
        try {
            ...
            // Create test patient
            Model.User patientUser = new Model.User("billpatient", "billpatient@example.com",
                                                    "Bill Patient", Util.PasswordHasher.hash("TestPassword123".toCharArray()),
                                                    Model.Role.PATIENT);
            Model.User savedPatientUser = userRepo.save(patientUser);
            java.time.LocalDate dob = java.time.LocalDate.of(1990, 1, 1);
            Model.Patient patient = new Model.Patient(savedPatientUser, "PT-BILL-001", dob, "M",
                                                      "O+", "Single", "123 Test St", "555-1234",
                                                      "John Doe", "555-0000");
            Model.Patient savedPatient = patientRepo.save(patient);
            ...
            // DELETE
            if (!billingRepo.delete(saved.getId())) return false;
